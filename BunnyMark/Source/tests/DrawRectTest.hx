...
class DrawRectTest extends Sprite 
{
	...
	{	
	 	...
	 	for (i in 0...numBunnies)
		{
			...
			matrix.tx = bunny.position.x;
			matrix.ty = bunny.position.y;
			
			graphics.beginBitmapFill (bunnyAsset, matrix);
			graphics.drawRect (bunny.position.x, bunny.position.y, bunnyAsset.width, bunnyAsset.height);
		}
